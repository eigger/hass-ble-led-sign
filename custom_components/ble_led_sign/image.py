"""Support for BLE LED sign label preview and last-sent image."""
...
async def async_setup_entry(
    ...
    if not driver_supports(hass, entry.entry_id, "supports_image"):
        return
    ...
    async_add_entities(
        [
            BleLedSignImageEntity(hass, entry, image_coordinator),
            BleLedSignPreviewImageEntity(hass, entry, preview_coordinator),
        ]
    )


class BleLedSignImageEntity(CoordinatorEntity[DataUpdateCoordinator[bytes | None]], ImageEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"ble_led_sign_{self._identifier}_last_updated_content"
...
class BleLedSignPreviewImageEntity(
    ...
    def __init__(
        ...
        self._address = address
        self._identifier = address.replace(":", "")[-8:].upper()
        self._attr_unique_id = f"ble_led_sign_{self._identifier}_preview_content"
        self._attr_content_type = "image/png"
        self._cached_image = Image(content_type="image/png", content=coordinator.data)
